...
let score = 0;
let maxScore = 0; // Se calcula dinámicamente según el total de preguntas
...
function startGame() {
    ...
    // Calcular puntaje máximo posible (10 pts por pregunta)
...
function processTextWithGlossary(text) {
    ...
    // que "IA" reemplace parte de "Inteligencia Artificial" si ambas están en el glosario.
    const terminos = Object.keys(glosarioTerminos).sort((a, b) => b.length - a.length);

    terminos.forEach(termino => {
        const definicion = glosarioTerminos[termino];
        // Regex: \b asegura que sea palabra completa, 'gi' es global e insensible a mayúsculas
        const regex = new RegExp(`\\b(${termino})\\b`, 'gi');
        
        // Reemplazamos por un span con tooltip. $1 mantiene la capitalización original del texto.
...
function loadQuestion() {
    ...
    // (Lógica simplificada para la barra: preguntas totales vs actuales)
    ...
    preguntaActual.opciones.forEach((opcion, index) => {
        ...
        // También procesamos el glosario en las opciones
...
function checkAnswer(selectedIndex, correctIndex) {
    ...
    // Mostrar cuál era la correcta y la incorrecta
    if (selectedIndex === correctIndex) {
        ...
        botones[correctIndex].classList.add('correct'); // Mostrar cuál era la buena
...
function endGame() {
    ...
    if (porcentaje === 100) {
        mensaje = "🏆 ¡Rango: PILOTO EXPERTO! Tienes el control total. La IA es tu herramienta, no tu jefe.";
    } else if (porcentaje >= 80) {
        mensaje = "🥇 ¡Rango: PILOTO AVANZADO! Tienes muy buen criterio, pero mantente alerta a las alucinaciones.";
    } else if (porcentaje >= 60) {
        mensaje = "🥈 ¡Rango: COPILOTO EN ENTRENAMIENTO! Entiendes los riesgos, pero a veces confías demasiado.";
    } else {
        mensaje = "⚠️ ¡Rango: PASAJERO AUTOMÁTICO! Estás en riesgo de manipulación o error. Revisa el material teórico y vuelve a intentarlo.";
    }
